                      ...
                      {#if row.left.type !== 'empty'}
                        ...
                        <span class="diff-line-code">
                          {#if row.left.segments}
                            {#each row.left.segments as seg}
                              <span class={seg.type === 'change' ? 'intraline-change' : ''}>{seg.text}</span>
                            {/each}
                          {:else}
                            {row.left.content}
                          {/if}
                      ...
                      {#if row.right.type !== 'empty'}
                        ...
                        <span class="diff-line-code">
                          {#if row.right.segments}
                            {#each row.right.segments as seg}
                              <span class={seg.type === 'change' ? 'intraline-change' : ''}>{seg.text}</span>
                            {/each}
                          {:else}
                            {row.right.content}
...
<style lang="scss">
  ...
  /* Intra-line highlighting (GitHub-like) */
  .diff-line-add .intraline-change { background-color: rgba(34, 197, 94, 0.35); border-radius: 2px; }
  .diff-line-del .intraline-change { background-color: rgba(239, 68, 68, 0.35); border-radius: 2px; }
</style>
